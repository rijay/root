  ...
  <section class="workbench content-workbench health-operations-page health-recommendations-page">
    ...
    <el-drawer v-model="drawerVisible" class="content-edit-drawer health-edit-drawer" size="408px" :show-close="true">
      ...
      <el-form label-position="top">
        <el-form-item label="主分类 *"><el-select v-model="draft.primaryCategory" placeholder="选择建档主分类"><el-option label="肠道规律关注型" value="BOWEL" /><el-option label="腹胀反酸关注型" value="DIGESTION" /><el-option label="睡眠节律关注型" value="SLEEP" /><el-option label="压力活力关注型" value="ENERGY" /><el-option label="活动饮食调整型" value="LIFESTYLE" /><el-option label="基础状态维护型" value="BASELINE" /></el-select></el-form-item>
...
import { fetchHealthScales, fetchRecommendationRules, saveRecommendationRuleDraft } from "./adminHealthApi";

const emptyDraft = () => ({ id: "", sourceVersionId: "", primaryCategory: "", auxiliaryTagsText: "", matchSummary: "", priority: 10, matchMode: "ANY", maxRecommendations: 3, scaleVersionId: "", effectiveAt: "" });
...
function editDraft(row) { Object.assign(draft, emptyDraft(), row, { id: row.status === "PUBLISHED" ? "" : row.id, sourceVersionId: row.status === "PUBLISHED" ? row.versionId : row.sourceVersionId || "", auxiliaryTagsText: (row.auxiliaryTags || []).join("\n") }); drawerVisible.value = true; }
...
async function saveDraft() {
  ...
  try { await saveRecommendationRuleDraft({ id: draft.id, sourceVersionId: draft.sourceVersionId, primaryCategory: draft.primaryCategory, auxiliaryTags, matchSummary: draft.matchSummary.trim(), priority: draft.priority, matchMode: draft.matchMode, maxRecommendations: draft.maxRecommendations, scaleVersionId: draft.scaleVersionId, effectiveAt: draft.effectiveAt }); ElMessage.success("推荐规则草稿已保存"); drawerVisible.value = false; await load(); }
