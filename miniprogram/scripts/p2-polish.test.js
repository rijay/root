...
assert.match(productStyles, /\.product-carousel\s*\{[^}]*height:\s*862rpx/s);
assert.equal(productsConfig.navigationStyle, "custom");
assert.equal(productsConfig.backgroundColor, "#FDFBF6");
...
assert.doesNotMatch(products, /会员中心实时价格/);
assert.doesNotMatch(productStyles, /\.product-learn/);
assert.doesNotMatch(productStyles, /\.product-price/);
...
assert.match(productStyles, /button\.product-buy\s*\{[^}]*width:\s*100%\s*!important/s);
