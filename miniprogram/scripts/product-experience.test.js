...
assert.match(listScript, /prepareProductJump/);
assert.match(listScript, /sourcePage:\s*"\/pages\/products\/index"/);
...
assert.match(listWxml, /\{\{catalogNotice\}\}/);
assert.match(listWxml, /item\.priceLive/);
assert.doesNotMatch(listWxml, /class="product-spec"/);
assert.doesNotMatch(listWxml, /item\.syncedAtText/);
assert.doesNotMatch(listWxml, /有赞同步/);
assert.doesNotMatch(listScript, /presentCatalogRefreshStatus/);
assert.doesNotMatch(listScript, /商品更新失败，已显示本地信息/);
assert.match(detailScript, /product_detail_view/);
assert.match(detailScript, /sourcePage:\s*"\/pages\/product-detail\/index"/);
assert.match(detailScript, /member_center_handoff/);
assert.match(detailScript, /prepareProductJump/);
assert.match(detailScript, /sourcePage:\s*"\/pages\/product-detail\/index"/);
