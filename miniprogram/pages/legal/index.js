  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        title: "三、模型辅助建议",
        items: [
          "健康建议采用预先生成并经人工审核的通用建议目录。CloudBase AI 只在内容制作阶段接收产品团队定义的合成状态场景，不接收任何真实用户的身份、问卷答案、评测结果、健康状态或请求日志。",
          "你使用小程序时，myRoot 服务端仅按评测结果在本地目录中匹配建议，不会发起实时模型调用。目录缺失、未完整审核或内容无效时自动改用经审核固定建议；安全提示分支始终使用经审核的安全文案。",
        ],
      },
      {
        title: "四、你的选择与权利",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "健康建议目录匹配：仅在 myRoot 服务端使用评测结果代码选择经审核建议；不向模型提供用户身份、联系方式、评测结果、原始问卷答案或自由文本。",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "CloudBase AI 仅用于离线制作通用建议目录，输入为产品团队预定义的合成状态场景，不来自用户请求，也不包含用户个人信息。线上运行时不调用该模型服务。",
        ],
      },
      {
        title: "Root 会员中心",
