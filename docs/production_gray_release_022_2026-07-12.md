...
1. 022 的 CloudBase 对象存储上传/删除候选探针已执行但返回 HTTP 502，未取得上传确认；目录回读为 0 个对象，未发现残留。该候选随后取消灰度，不再承接任何路由。
...
022 已作为失败证据关闭，不等于正式上线完成。

## 8. 对象存储失败与关闭证据

1. 022 定向对象存储探针进入后端后返回 HTTP 502，未返回可确认的对象引用；候选容器仍为 `normal`，稳定版流量未改变。
2. 探针失败后直接列举 `release-probes/2026-07-12/`，结果为 `total=0`，没有发现需要人工清理的残留对象。
3. 根因定位为 CloudRun 中使用 CloudBase Node SDK 的对象存储路径不适用于当前运行形态；后续候选改为服务端 CloudBase HTTP 存储 Interface，并新增上传授权、精确 PUT、按返回 `cloudObjectId` 删除及含糊上传补偿删除测试。
4. 2026-07-12 19:30 +08:00 在控制台执行“取消灰度”，明确把 `myroot-api-012` 设置为 100% 并结束 022 灰度。发布单回读 `IsReleasing=false`，部署记录回读 012 为 `100% / HasTraffic=true`、022 为 `0% / HasTraffic=false`。
5. 022 没有删除，保留用于复盘；后续验证使用独立候选 `myroot-api-023`，详见 [候选 023 证据](./production_gray_release_023_2026-07-12.md)。
