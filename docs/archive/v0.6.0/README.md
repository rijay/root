...
> 已提交版本标签：`archive/v0.6.0-review-submitted-20260825`
> 本地调整标签：`archive/v0.6.0-post-submit-adjustment-20260825`
> 分支头状态：`LOCAL_ADJUSTED / REUPLOAD_REQUIRED`

## 1. 归档结论

本目录冻结 myRoot v0.6.0 已提交审核版本及其提审后本地调整的代码、配置、测试记录和交接边界，供审核问题定位、版本回溯和下一版本开发使用。

`archive/v0.6.0-review-submitted-20260825` 固定指向 12:05 已提交微信审核的原候选。当前分支头随后增加了两项本地调整，尚未重新上传或重新提交审核：

- 肠道五类结果的第一条纤维建议改为分类文案，结果文案版本由 v4 升为 v5；
- 首页 Banner3 的两行副文案字号由 `44rpx` 调整为 `36rpx`。

该归档只证明：

- 项目负责人确认 2026-08-25 12:05 上传的 `0.6.0` 候选已被设置为体验版并完成真机冒烟；
- 项目负责人确认已手动提交微信审核；
- 已提交标签和当前本地调整代码均保留在 Git 历史中；
- 当前分支头已通过本地自动化检查；
...
```bash
git switch --detach archive/v0.6.0-post-submit-adjustment-20260825
```
