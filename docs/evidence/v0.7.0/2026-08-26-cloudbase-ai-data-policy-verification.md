...
- 项目接受服务端临时缓存最长 5 分钟作为内部上限，但当前账户页面没有对应配置项，公开材料还明确表示缓存失效没有固定规则，不能证明该上限对本账户具有硬约束。因此 `ROOT_HEALTH_ADVICE_MODEL_DATA_POLICY_VERIFIED` 仍不得设为 `true`，正式模型调用继续 fail-close，固定建议降级路径不受影响。
