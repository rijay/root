...
状态：已部署为 `myroot-api-024 / URL_PARAMS / 0%` 条件候选，两个 Cloud Function 代码包已对齐到 0.5.7；尚未上传小程序、进入百分比灰度或正式切流
...
9. `myroot-job-dispatcher` 与 `myroot-health-retention` 已使用只更新代码的 CLI Interface 对齐到 0.5.7；每个函数各 6 个变量、两函数合计 10+1 个启用触发器和全局 dry-run 未漂移，11/11 作业均返回 `releaseVersion=0.5.7 / dryRun=true / HTTP 200 / code=0`。对齐后 15 次无参数 `/health` 仍未命中候选。
