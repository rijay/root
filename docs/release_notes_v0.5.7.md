...
关闭 `v0.5.6 / myroot-api-023` 正式 Gate 审计中新发现的有赞奖励 Adapter 契约与外部响应持久化风险，不覆盖已部署的 023 工件。
...
2. 未执行真实发券、券状态查询、企微写入、体验版上传、CloudRun 部署、5% 灰度或正式发布。
3. `myroot-api-012` 继续承接默认流量，`myroot-api-023 / v0.5.6` 继续保持 0% URL 参数候选状态。
