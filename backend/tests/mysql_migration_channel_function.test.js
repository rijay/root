...
test("package builder copies the immutable 75 migration set", () => {
  ...
  try {
    ...
    assert.equal(result.migrationCount, 75);
    ...
    assert.equal(fs.existsSync(path.join(output, "db", "migrations", "075_user_labels.sql")), true);
  } finally {
