...
test("release evidence pack summarizes launch evidence without leaking secrets", () => {
  ...
  assert.equal(pack.evidence.actionAdapterCalibration.actions.length, 4);
  ...
  assert.equal(pack.evidence.productionCutoverReadiness.summary.requiredProofCount, 14);
  ...
  assert.equal(pack.evidence.productionEvidenceIntake.items.length, 14);
