...
function buildProductionEvidenceIntake(input = {}) {
  ...
  const rows = [
    ...
    item({
      ...
      backlogId: "T-006",
      group: "operations",
      label: "企微咨询跳转和联系回写真执行证据",
      ownerRole: "研发/运营",
      status: worstStatus([
        adapterSourceStatus(input.adapterCalibration, ["WECHAT_LEAD"]),
        actionStatus(input.actionAdapterCalibration, ["WEWORK_CONTACT_WRITEBACK"]),
      ...
      signals: [
        ...
        ...actionSignals(input.actionAdapterCalibration, ["WEWORK_CONTACT_WRITEBACK"]),
