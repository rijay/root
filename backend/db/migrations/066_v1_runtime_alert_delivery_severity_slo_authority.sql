-- Recreate the two registration procedures so the database authority derives
-- the accepted delivery SLO from the persisted runtime-alert severity. A
-- registrar cannot downgrade a BLOCKER alert to a valid WARNING delivery SLO.
...
BEGIN
  ...
  IF v_existing_delivery_id IS NULL THEN
    ...
    WHERE authority.environment_id = p_environment_id
      ...
      AND (
        (
          runtime_alert.severity = 'BLOCKER'
          AND p_slo_class = 'BLOCKER_IMMEDIATE'
          AND p_slo_target_seconds = 300
        )
        OR (
          runtime_alert.severity = 'WARNING'
          AND p_slo_class = 'WARNING_STANDARD'
          AND p_slo_target_seconds = 1800
        )
      );
    IF ROW_COUNT() <> 1 THEN
