-- Enforce all-or-none recipient binding metadata on the v1 grant table. This
-- migration owns exactly one permanent ALTER because MySQL DDL implicitly
-- commits.
DROP TEMPORARY TABLE IF EXISTS migration_057_notification_recipient_v1_preflight;

CREATE TEMPORARY TABLE migration_057_notification_recipient_v1_preflight (
  guard_id TINYINT UNSIGNED NOT NULL,
  PRIMARY KEY (guard_id)
) ENGINE=InnoDB;

INSERT INTO migration_057_notification_recipient_v1_preflight (guard_id) VALUES (1);

INSERT INTO migration_057_notification_recipient_v1_preflight (guard_id)
SELECT 1
...
DROP TEMPORARY TABLE migration_057_notification_recipient_v1_preflight;

ALTER TABLE notification_subscription_grant_v1
  MODIFY COLUMN recipient_binding_status VARCHAR(16)
    CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'UNVERIFIED',
  ADD KEY idx_notification_recipient_binding_v1_crypto (
    recipient_binding_digest_scheme,
    recipient_binding_key_id,
    notification_subscription_grant_id
  ),
  ADD KEY idx_notification_recipient_identity_v1 (
    recipient_wechat_identity_id,
    recipient_binding_status,
    notification_subscription_grant_id
  ),
  ADD CONSTRAINT chk_notification_recipient_binding_v1
    CHECK (
      (
        recipient_binding_status = 'UNVERIFIED'
        AND recipient_wechat_identity_id IS NULL
