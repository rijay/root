...
function defaultRequestId(date = new Date()) {
  return `lifecycle-settlement-due-${date.toISOString().slice(0, 16).replace(/[-:T]/g, "")}`;
}
...
async function postLifecycleSettlementJob(args) {
  const body = {
    dryRun: args.dryRun,
  ...
  const response = await fetch(`${args.baseUrl}/api/v1/jobs/lifecycle-settlement-due`, {
    method: "POST",
    headers,
...
async function collectLifecycleSettlementJob(args) {
  return postLifecycleSettlementJob(args);
}

function formatJob(job) {
  if (!job) return "-";
  const summary = job.summary || {};
  return `${job.jobId || "-"} / ${job.status || "-"} / ${job.campaignId || "-"} / ${summary.processed || 0}/${summary.selected || 0}`;
}

function buildLifecycleSettlementJobReport(bundle) {
  const data = bundle.data || {};
  const lines = [
    "# ROOT 生命周期结算队列 Job 报告",
    "",
    ...
    "",
    "## 调度摘要",
    `- 候选：${data.selectedCount || 0} / 可执行总数：${data.eligibleCount || 0}`,
    ...
    "",
    "## 候选队列",
    ...((data.candidates || []).length ? data.candidates.map((job) => `- ${formatJob(job)}`) : ["- 暂无候选"]),
    "",
    "## 执行结果",
    ...((data.results || []).length
      ? data.results.map((item) => `- ${item.ok ? "OK" : "FAILED"} ${formatJob(item.job)}${item.error_message ? `：${item.error_message}` : ""}`)
      : ["- 暂无执行结果"]),
  ];
  return `${lines.join("\n")}\n`;
}
...
async function main() {
  ...
  try {
    const bundle = await collectLifecycleSettlementJob(args);
    if (args.json) {
      process.stdout.write(`${JSON.stringify(bundle, null, 2)}\n`);
    } else {
      process.stdout.write(buildLifecycleSettlementJobReport(bundle));
    }
    process.exitCode = determineExitCode(bundle);
  } catch (error) {
    process.stderr.write(`生命周期结算队列 Job 失败：${error.message}\n`);
    process.exitCode = 1;
  }
}
